...
import { Camera, Video, Type, Hash, ArrowLeft } from 'lucide-react';
...
const CreateTab: React.FC<CreateTabProps> = ({ onLogout }) => {
  const [postType, setPostType] = useState<'text' | 'image' | 'video' | null>(null);
  ...
  const handlePost = async () => {
    ...
    if (!content.trim() && !file) {
      toast({ title: "Validation Error", description: "Your post needs some content or a media file.", variant: "destructive" });
      return;
    }
    if (!postType) {
