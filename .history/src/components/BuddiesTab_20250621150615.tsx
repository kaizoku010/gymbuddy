...
const BuddiesTab: React.FC = () => {
  ...
  const fetchNearbyUsers = async () => {
    ...
    try {
      ...
      // Refined filtering logic: prefer city match, fallback to country
      let filtered = (data || []).filter((u: any) => {
        if (userCity && u.city) {
          return u.city === userCity && u.country === userCountry;
        } else if (userCountry && u.country) {
          return u.country === userCountry;
        }
        return false;
      });
